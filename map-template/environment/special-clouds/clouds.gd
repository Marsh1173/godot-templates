...
func update_cloud_textures():
	# cloud_*_texture uniforms live in cloud_functions.gdshaderinc, which is
	# only included by cloud_sdf.gdshader (the $clouds material_override).
	if $clouds == null:
		return
	$clouds.material_override.set_shader_parameter("cloud_base_texture", cloud_base_texture)
	$clouds.material_override.set_shader_parameter("cloud_detail_texture", cloud_detail_texture)
	$clouds.material_override.set_shader_parameter("cloud_volume_texture", cloud_volume_texture)

func update_cloud_globals():
	# Cloud shape and wind uniforms are declared in cloud_functions.gdshaderinc,
	# included only by cloud_sdf.gdshader. They go on $clouds.material_override.
	if $clouds == null:
		return
	$clouds.material_override.set_shader_parameter("cloud_base_height", cloud_base_height)
	$clouds.material_override.set_shader_parameter("cloud_base_scale", cloud_base_scale)
	$clouds.material_override.set_shader_parameter("cloud_base_squeeze", cloud_base_squeeze)
	$clouds.material_override.set_shader_parameter("cloud_detail_scale", cloud_detail_scale)
	$clouds.material_override.set_shader_parameter("cloud_detail_amount", cloud_detail_amount)
	$clouds.material_override.set_shader_parameter("cloud_detail_dist", cloud_detail_dist)
	$clouds.material_override.set_shader_parameter("cloud_volume_scale", cloud_volume_scale)
	$clouds.material_override.set_shader_parameter("cloud_volume_density_low", cloud_volume_density_low)
	$clouds.material_override.set_shader_parameter("cloud_volume_density_high", cloud_volume_density_high)
	$clouds.material_override.set_shader_parameter("wind_vector", wind_vector)
	$clouds.material_override.set_shader_parameter("wind_uplift", wind_uplift)
...
func update_world_environment():
	if !world_environment:
		...
		$buffer_sky/sky.material_override.set_shader_parameter("rayleigh", 2.0 )
		$buffer_sky/sky.material_override.set_shader_parameter("rayleigh_color", Color("4d6799") )
		$buffer_sky/sky.material_override.set_shader_parameter("mie", 0.005 )
		$buffer_sky/sky.material_override.set_shader_parameter("mie_eccentricity",0.8 )
		#sky_material.mie_color = directional_light.light_color
		$buffer_sky/sky.material_override.set_shader_parameter("mie_color", Color("ffcf8d") )
		$buffer_sky/sky.material_override.set_shader_parameter("turbidity", 10 )
		$buffer_sky/sky.material_override.set_shader_parameter("sun_disk_scale", 1.0 )
		$buffer_sky/sky.material_override.set_shader_parameter("ground_color", Color("1a1209") )
		$buffer_sky/sky.material_override.set_shader_parameter("exposure", 1.0 )
...
func update_directional_light():
	if !directional_light:
		...
		# sun_light_* is declared in both buffer_sky.gdshader and cloud_functions.gdshaderinc,
		# so it must be pushed to both materials.
		$buffer_sky/sky.material_override.set_shader_parameter("sun_light_direction", light_direction)
		$buffer_sky/sky.material_override.set_shader_parameter("sun_light_energy", directional_light.light_energy)
		$buffer_sky/sky.material_override.set_shader_parameter("sun_light_color", directional_light.light_color)
		$clouds.material_override.set_shader_parameter("sun_light_direction", light_direction)
		$clouds.material_override.set_shader_parameter("sun_light_energy", directional_light.light_energy)
		$clouds.material_override.set_shader_parameter("sun_light_color", directional_light.light_color)
...
func update_cloud_positions():
	if camera == null:
		pass
		#assert(false, "Error! Clouds need to be linked to a Camera3D node")
	else:
