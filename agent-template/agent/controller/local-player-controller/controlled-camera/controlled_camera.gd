...
func _process(delta):
	var desired_point: Vector3 = ray_cast_3d.global_position + ray_cast_3d.global_transform.basis.z * max_range
	if ray_cast_3d.is_colliding():
		desired_point = ray_cast_3d.get_collision_point() - (ray_cast_3d.global_transform.basis.z.normalized() * collision_margin)
	
	if ray_cast_3d.global_position.distance_to(desired_point) < ray_cast_3d.global_position.distance_to(camera_3d.global_position):
